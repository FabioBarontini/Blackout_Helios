...
create table if not exists public.teams (
  ...
  class_level smallint not null default 2 check (class_level in (2,3)),
  created_at timestamptz not null default now()
);
...
create table if not exists public.verdicts (
  ...
  alternative_suspect text,
  limits text,
  timeline text,
  alternative_score integer not null default 0,
  limits_score integer not null default 0,
