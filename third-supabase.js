...
(function(){
  ...
  function updateTeamUI(){
    ...
    if(badge){badge.className='team-badge'+(logged?' open':'');badge.textContent=logged?'◉ '+currentTeam.name+' · 3ª':''}
  ...
  window.showPage=function(id){
    if(['briefing','labs','suspects','suspect-detail','guide','submit'].includes(id)&&!currentTeam){location.href='index.html?class=3';return}
    originalShow(id);
  ...
  window.submitVerdict=async function(){
    ...
    await loadProgress();
    const code=unlockCode||'BLACKOUT-047',suspect=document.getElementById('who').value,motivation=document.getElementById('motivation').value,other=document.getElementById('motivationOther').value.trim(),how=document.getElementById('how').value.trim(),proof=document.getElementById('proof').value.trim(),confidence=document.getElementById('confidence').value,timeline=document.getElementById('timeline')?.value.trim()||'',alternative=document.getElementById('alternative')?.value||'',limits=(document.getElementById('limits')?.value.trim()||'')+' '+(document.getElementById('evidenceLimits')?.value.trim()||'');
    if(!code||!suspect||!motivation||!how||!proof||!timeline||!alternative||!limits.trim()){status('submit-status','Compila sospettato, motivazione, ricostruzione, prove, alternativa, ricostruzione temporale e limiti dell’evidenza.','bad');return}
    if(alternative===suspect){status('submit-status','Il sospetto alternativo deve essere diverso da quello indicato come responsabile.','bad');return}
    ...
    const {data,error}=await client.rpc('submit_verdict_v2',{p_code:code,p_suspect:suspect,p_motivation:motivation,p_motivation_other:other,p_how:how,p_proof:proof,p_confidence:confidence,p_alternative_suspect:alternative,p_limits:limits,p_timeline:timeline});
  ...
  async function loadAdminDashboard(){
    ...
    const {data:teams,error}=await client.from('teams').select('id,name,class_level,created_at,team_members(name,position),lab_progress(lab_id,score,completed),verdicts(suspect,motivation,motivation_other,how,proof,confidence,alternative_suspect,limits,timeline,suspect_score,motivation_score,reasoning_score,technical_score,alternative_score,limits_score,timeline_score,verdict_score,lab_score,total_score,submitted_at)').order('created_at',{ascending:true});
  ...
  async function init(){
    ...
    if(!currentUser){location.href='index.html?class=3';return;}
    try{
      const loaded=await loadTeam(currentUser);
      if(loaded.profile?.role==='admin'){updateTeamUI();showPage('admin');return;}
      if(loaded.team){updateTeamUI();showPage('briefing');return;}
      location.href='index.html?class=3';
    }catch(e){console.warn(e);location.href='index.html?class=3';}
  }
  client.auth.onAuthStateChange((event,session)=>{
    currentUser=session?.user||null;
    if(event==='SIGNED_OUT'){location.href='index.html?class=3';}
...
// THREE EXTRA DISCOVERY CHALLENGES — construction, delayed verification, no instant answer feedback.
(function(){
 const $=id=>document.getElementById(id), state={A:[],B1:[],B2:[],C:[]};
 const makeToken=(text,key,kind='extra-token')=>{const d=document.createElement('div');d.className=kind;d.textContent=text;d.draggable=true;d.dataset.key=key;d.addEventListener('dragstart',e=>{window.__blackoutDrag={key,text};e.dataTransfer.setData('text/plain',key)});return d};
 const setupDrop=(el,onDrop)=>{el.addEventListener('dragover',e=>{e.preventDefault();el.classList.add('over')});el.addEventListener('dragleave',()=>el.classList.remove('over'));el.addEventListener('drop',e=>{e.preventDefault();el.classList.remove('over');const d=window.__blackoutDrag;if(d)onDrop(d)})};
 const renderList=(el,arr)=>{el.innerHTML='';if(!arr.length){el.innerHTML='<div class="extra-placeholder">Trascina qui gli elementi.</div>';return}arr.forEach((x,i)=>{const d=makeToken(x.text,x.key,'extra-attached');d.title='Clicca per rimuovere';d.addEventListener('click',()=>{arr.splice(i,1);renderList(el,arr)});el.appendChild(d)})};
 const AT=[['Marta','marta'],['rete locale','lan'],['dispositivo che collega reti diverse','router'],['rete Aurora','aurora'],['server Aurora','server'],['servizio che traduce un nome','dns'],['computer della stessa rete','switch']];
 const atEl=$('extraATokens'),aPath=$('extraAPath'); if(atEl&&aPath){AT.forEach(([t,k])=>atEl.appendChild(makeToken(t,k)));setupDrop(aPath,d=>{if(state.A.some(x=>x.key===d.key))return;state.A.push({key:d.key,text:d.text});renderList(aPath,state.A)});$('extraAVerify').onclick=()=>{const ok=state.A.map(x=>x.key).join('|')==='marta|lan|router|aurora|server',e=$('extraAres');e.className='lab-result '+(ok?'ok':'bad');e.textContent=ok?'✓ Percorso coerente. Fermati qui: il nome tecnico lo darà il docente.':'✗ La configurazione non è coerente. Il sistema non indica quale elemento va cambiato: ricostruisci il percorso e verifica di nuovo.'};$('extraAReset').onclick=()=>{state.A=[];renderList(aPath,state.A)}}
 const BT=[['Marta','marta'],['Luca','luca'],['identificativo 53124','p53124'],['identificativo 53125','p53125'],['protocollo TCP','tcp'],['file','file'],['messaggio','msg'],['colore PC','color'],['lunghezza cavo','cable']];
 const btEl=$('extraBTokens'),chat=$('extraBChat'),file=$('extraBFile'); if(btEl&&chat&&file){BT.forEach(([t,k])=>btEl.appendChild(makeToken(t,k)));setupDrop(chat,d=>{if(state.B1.some(x=>x.key===d.key))return;state.B1.push({key:d.key,text:d.text});renderList(chat,state.B1)});setupDrop(file,d=>{if(state.B2.some(x=>x.key===d.key))return;state.B2.push({key:d.key,text:d.text});renderList(file,state.B2)});$('extraBVerify').onclick=()=>{const a=state.B1.map(x=>x.key),b=state.B2.map(x=>x.key),r=$('extraBReason').value.trim(),ok=a.includes('p53124')&&a.includes('msg')&&b.includes('p53125')&&b.includes('file')&&r.length>=25,e=$('extraBres');e.className='lab-result '+(ok?'ok':'bad');e.textContent=ok?'✓ Le due comunicazioni sono state costruite e motivate. Fermati qui: il nome tecnico lo darà il docente.':'✗ Le due comunicazioni non sono ancora distinguibili in modo convincente. La verifica non indica quale elemento sia errato: controlla l’identificazione dei due flussi e spiega perché.'};$('extraBReset').onclick=()=>{state.B1=[];state.B2=[];$('extraBReason').value='';renderList(chat,state.B1);renderList(file,state.B2)}}
 const seq=$('extraCSequence'), reason=$('extraCReason'); if(seq){document.querySelectorAll('.trace-item').forEach(x=>x.addEventListener('dragstart',()=>window.__blackoutTrace=x));setupDrop(seq,()=>{const x=window.__blackoutTrace;if(!x||state.C.includes(x.dataset.trace))return;state.C.push(x.dataset.trace);const c=x.cloneNode(true);c.className='extra-attached';c.draggable=false;c.title='Clicca per rimuovere';c.onclick=()=>{const i=state.C.indexOf(x.dataset.trace);if(i>=0)state.C.splice(i,1);c.remove()};seq.querySelector('.extra-placeholder')?.remove();seq.appendChild(c);window.__blackoutTrace=null});$('extraCVerify').onclick=()=>{const ok=state.C.join('|')==='dns|internal|external|external2'&&reason.value.trim().length>=35,e=$('extraCres');e.className='lab-result '+(ok?'ok':'bad');e.textContent=ok?'✓ Hai costruito una sequenza che merita analisi. Fermati qui: il docente guiderà l’interpretazione.':'✗ La sequenza non è ancora sufficientemente coerente. Il sistema non indica quale evento spostare: osserva tempi, sorgente, destinazione e relazione tra gli eventi.'};$('extraCReset').onclick=()=>{state.C=[];reason.value='';seq.innerHTML='<div class="extra-placeholder">Trascina qui quattro eventi. L’ordine deve raccontare una sequenza coerente.</div>'}}
})();
