...
// THREE EXTRA DISCOVERY CHALLENGES — no score, preparation for PCAP analysis.
(function(){
  const bind=(id,correct,res)=>{document.querySelectorAll('#'+id+' .lab-choice').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('#'+id+' .lab-choice').forEach(x=>x.classList.remove('good','bad'));const ok=b.dataset.v===correct;b.classList.add(ok?'good':'bad');const e=document.getElementById(res);if(e){e.className='lab-result '+(ok?'ok':'bad');e.textContent=ok?'✓ Ipotesi coerente. Fermati qui: il nome tecnico lo darà il docente.':'✗ Non risolve il problema descritto. Rileggi la situazione e prova a separare il problema dalla soluzione.'}}))};
  bind('extraA','router','extraAres');bind('extraB','port','extraBres');bind('extraC','a','extraCres');
})();
